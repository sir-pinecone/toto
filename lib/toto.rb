...
module Toto
  ...
  class Site
    ...
    def archives filter = "", tag = nil
      ...
      if tag.nil?
        ...
        tag_name = !tagged.empty? && tagged.first[:tags].find { |t| t.slugize == tag }
  ...
  class Article < Hash
    ...
    def summary length = nil
      ...
      highlight_code(sum.length == self[:body].length ? sum : sum.strip.sub(/\.\Z/, '&hellip;'))
    end
    ...
    def reading_time
      ...
      reading_time = ""
      if minutes > 0
        reading_time = "#{minutes} #{min_title} & "
      end
  ...
  class Config < Hash
    Defaults = {
      ...
      :reading_wpm => 250,                                  # average reading speed in words per min
